...
export default function SourceC({ apps }: Props) {
  return (
    <div className="min-h-screen bg-black text-white py-6 px-4">
      <h1 className="text-2xl font-bold text-center mb-6">🔥 熱門 App 精選牆 - Source C 🔥</h1>
      {apps.length === 0 ? (
...
export async function getServerSideProps() {
  try {
    const res = await fetch('https://api.jsonbin.io/v3/b/6834889d8a456b7966a59913', {
      headers: {
    ...
    console.error('Error loading JSONBin (Source C):', err);
    return {
      props: {
        apps: []
      }
